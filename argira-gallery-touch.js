...
(function () {
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    // Dibujar imagen con letterbox (object-fit: contain) para preservar proporciones.
    // Se calculan offset y escala para centrar la imagen dentro del canvas 512×512.
    // offsetX, offsetY, drawW, drawH se usan también al leer píxeles en handleTouch.
    let offsetX = 0, offsetY = 0, drawW = SIZE, drawH = SIZE;

    function computeLetterbox() {
      const natW = cardImg.naturalWidth  || SIZE;
      const natH = cardImg.naturalHeight || SIZE;
    ...
    function drawImage() {
      ...
      ctx.drawImage(cardImg, offsetX, offsetY, drawW, drawH);
    }
    ...
    label.className = 'argira-color-label';
    ...
    canvas.addEventListener('keydown', (e) => {
      ...
      if (['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' ','Enter'].includes(e.key)) {
        ...
        const px = Math.round(offsetX + ((keyZone.col * 2 + 1) / 6) * drawW);
        const py = Math.round(offsetY + ((keyZone.row * 2 + 1) / 6) * drawH);
    ...
    function handleTouch(e) {
      ...
      // Coordenadas relativas al rect visible (0–1) — fuente única de verdad
      const relX = (clientX - rect.left) / rect.width;
      const relY = (clientY - rect.top)  / rect.height;

      // Leer píxel del canvas usando coordenadas ajustadas al letterbox
      const px = Math.round(offsetX + relX * drawW);
      const py = Math.round(offsetY + relY * drawH);
      ...
      const panVal = centroid.cx > 0.55 ? 'derecha' : centroid.cx > 0.52 ? 'centro-derecha' : centroid.cx < 0.45 ? 'izquierda' : centroid.cx < 0.48 ? 'centro-izquierda' : 'centro';
      label.textContent = `${texto}  ·  ${freq} Hz  ·  pan ${panVal}`;
      playTone(h, s, centroid.cx, centroid.cy);

      const textoHablado = `${texto}, ${freq} hercios, color dominante hacia ${panVal}`;
  ...
  function init() {
    loadCentroides(() => {
      ...
      // Lanzar widget de rugosidad una vez los labels .argira-color-label existen
      setTimeout(() => {
        if (typeof window.initRoughnessWidgets === 'function') window.initRoughnessWidgets();
      }, 200);
    });
  }
