...
(function () {
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    tmp.onload = () => {
      ...
      setTimeout(() => {
        try {
          ...
          console.warn('Argira: canvas tainted por CORS en', cardImg.src, '— centroide por defecto (cx=0.5)');
        }
      }, 120);
    };
    tmp.src = cardImg.src + (cardImg.src.includes('?') ? '&' : '?') + 'cors=1';
    ...
    function handleTouch(e) {
      ...
      const panLabel = centroid.cx > 0.55 ? 'derecha' : centroid.cx < 0.45 ? 'izquierda' : 'centro';
