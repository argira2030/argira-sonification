...
(function () {
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    function handleTouch(e) {
      ...
      const panVal   = centroid.cx > 0.52 ? 'derecha' : centroid.cx < 0.48 ? 'izquierda' : 'centro';
      const panIcon  = centroid.cx > 0.52 ? '→' : centroid.cx < 0.48 ? '←' : '·';
      label.textContent = `${texto}  ·  ${freq} Hz  ·  ${panIcon} ${panVal}`;
      playTone(h, s, centroid.cx, centroid.cy);
