/**
 * argira-gallery-touch.js
 * Añade exploración táctil con descripciones de objetos a las 16 cards de la galería.
...
(function () {
  ...
  function playTone(h, s) {
    try {
      const ctx = window.ArgiraAudio ? window.ArgiraAudio.resume() : new (window.AudioContext || window.webkitAudioContext)();
      const freq = 200 + h * 800;
      const vol  = 0.40 + s * 0.45;   // mínimo 0.40, máximo 0.85
      const dur  = 0.55;               // más largo, más perceptible
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    function handleTouch(e) {
      ...
      label.textContent = `${texto}  ·  ${freq} Hz`;
      playTone(h, s);
