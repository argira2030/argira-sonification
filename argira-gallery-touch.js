...
(function () {
   ...
   */
  function computeChromaticCentroid(ctx2d, width, height) {
    ...
    const cx = wSumXY > 0 ? xSum / wSumXY / width  : 0.5;
    const cy = wSumXY > 0 ? ySum / wSumXY / height : 0.5;
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    tmp.onload = () => {
      // Ajustar el alto del canvas a las proporciones reales de la imagen
      const aspect = tmp.naturalHeight / tmp.naturalWidth;
      canvas.height = Math.round(SIZE * aspect);
      ctx.drawImage(tmp, 0, 0, SIZE, canvas.height);
      // Pequeño delay para asegurar que el rasterizado está completo
      const canvasH = canvas.height;
      setTimeout(() => {
        try {
          centroid = computeChromaticCentroid(ctx, SIZE, canvasH);
