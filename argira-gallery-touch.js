...
(function () {
  ...
  function playTone(h, s, panX, pitchY) {
    try {
      ...
      // StereoPannerNode: panX 0–1 → pan −1…+1
      const pan = (panX !== undefined) ? (panX * 2 - 1) : 0;
  ...
  /**
   * Calcula el centro de masa cromático de la imagen ya dibujada en el canvas.
   * Usa circular mean para hue (evita el artefacto rojo/violeta en 0/1).
   * Pondera cada píxel por su saturación — los píxeles acromáticos no arrastran
   * el centroide.
   *
   * @param {CanvasRenderingContext2D} ctx2d  Contexto 2D del canvas
   * @param {number} size                     Lado del canvas cuadrado (píxeles)
   * @returns {{ cx: number, cy: number }}    Coordenadas normalizadas 0–1
   */
  function computeChromaticCentroid(ctx2d, size) {
    // Submuestreo para rendimiento: 1 de cada STEP píxeles en x e y
    const STEP = 4;
    let sinSum = 0, cosSum = 0, wSumH = 0;
    let xSum   = 0, ySum   = 0, wSumXY = 0;

    const imgData = ctx2d.getImageData(0, 0, size, size).data;
    ...
    for (let row = 0; row < size; row += STEP) {
      for (let col = 0; col < size; col += STEP) {
        ...
        // Para el centroide espacial, ponderar por saturación (colores vivos importan más)
        const wXY = sat;
        xSum    += col * wXY;
        ySum    += row * wXY;
        wSumXY  += wXY;

        // Circular mean del hue, también ponderado por saturación
        if (sat > 0.08) {
          const angle = h * Math.PI * 2;
          sinSum += Math.sin(angle) * sat;
          cosSum += Math.cos(angle) * sat;
          wSumH  += sat;
        }
      }
    }

    const cx = wSumXY > 0 ? xSum / wSumXY / size : 0.5;
    const cy = wSumXY > 0 ? ySum / wSumXY / size : 0.5;

    // hueMean queda disponible para futuros usos (firma tonal global)
    // const hueMean = wSumH > 0 ? (Math.atan2(sinSum / wSumH, cosSum / wSumH) / (Math.PI * 2) + 1) % 1 : 0.5;
  ...
  function attachTouchToCard(cardImg, objectMap) {
    ...
    wrap.style.cssText = 'position:relative;margin-top:8px;touch-action:none;';
    ...
    canvas.style.cssText = 'width:100%;height:auto;display:block;cursor:crosshair;border-radius:6px;';
    canvas.setAttribute('tabindex', '0');
    canvas.setAttribute('role', 'application');
    canvas.setAttribute('aria-label', 'Lienzo interactivo de la pintura. Usa las flechas del teclado o toca para explorar zonas de color y oír el elemento de cada zona.');

    const ctx = canvas.getContext('2d');

    // Centroide cromático — se calcula una vez al cargar la imagen
    ...
    tmp.onload = () => {
      ...
      // Pequeño delay para asegurar que el rasterizado está completo
      setTimeout(() => {
        try {
          centroid = computeChromaticCentroid(ctx, SIZE);
        } catch (e) { /* canvas tainted por CORS → centroide por defecto */ }
      }, 120);
    };
    tmp.src = cardImg.src;
    ...
    canvas.addEventListener('keydown', (e) => {
      ...
      if (['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' ','Enter'].includes(e.key)) {
        ...
        const py = Math.round((keyZone.row * 2 + 1) / 6 * SIZE);
    ...
    function handleTouch(e) {
      e.preventDefault();
      // Ignorar clicks sintéticos de teclado/AT (coordenadas 0,0 sin touches)
      if (e.type === 'click' && !e.touches && e.clientX === 0 && e.clientY === 0) return;
      ...
      label.textContent = `${texto}  ·  ${freq} Hz  ·  pan ${centroid.cx > 0.55 ? '→' : centroid.cx < 0.45 ? '←' : '·'}`;
      playTone(h, s, centroid.cx, centroid.cy);

      setTimeout(() => speak(texto), 450);
    }
