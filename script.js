...
console.log('ARGIRA SCRIPT VERSION 2026-05-31-D');
  ...
  speak(text, opts = {}) {
    ...
    const doSpeak = () => {
      ...
      function speakNext(chunkRetries) {
        ...
        // chunkRetries cuenta reintentos del chunk actual — se pasa explícitamente
        // para que no se pierda al crear una nueva SpeechSynthesisUtterance.
        const retries = chunkRetries || 0;
        const chunk = self._queue.shift();
        const u = new SpeechSynthesisUtterance(chunk);
        // Guardamos el texto completo original en current para que el toggle funcione
        ...
        u.onerror = (e) => {
          ...
          // Reintentar el mismo chunk (pasando retries+1) hasta 3 veces.
    ...
    // waitForSpeech no se resuelva prematuramente durante los 90ms de espera.
    if (/iP(hone|ad|od)/.test(navigator.userAgent)) {
      this.speaking = true; // bloquear waitForSpeech durante el tick de espera
      setTimeout(() => {
        // doble check: si aún sigue hablando tras cancel, esperar un tick más
...
function normalizeBoxes(boxes) {
  ...
  console.log('[Argira] normalizeBoxes entrada:', boxes.length, '-> salida:', result.length, result);
  return result;
}
...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  if (best) console.log('[Argira] DIRECT HIT ->', best.label);
  ...
  if (!best) {
    ...
    if (best) {
      console.log('[Argira] FALLBACK HIT dist=%.3f -> %s', bestDist, best.label);
    }
  }

  console.log('[Argira] findObjectAtPoint px=%d py=%d -> %s', px, py, best ? best.label : 'null');
    ...
    function fetchObjectMap() {
      ...
      try {
        ...
        fetch(PROXY_URL, {
          ...
          card._argiraMapFetched = true;
          console.log('[Argira] objectMap galería:', JSON.stringify(data));
        }).catch((err) => {
          clearTimeout(_tid1);
          objectMap = null;
      ...
      async function runTour() {
        for (let idx = 0; idx < sequence.length; idx++) {
          ...
          if (_tourSpeed === 2) {
            ...
            // esperar señal explícita de cola terminada
            const maxWait = _tourSpeed === 0 ? 15000 : 8000;
            await window.ArgiraSpeech.waitUntilFinished(maxWait);
            if (_aborted || _tourId !== myTourId) return;
...
async function fetchSharedObjectMap(imageElement) {
  ...
  try {
    ...
    try {
      ...
      if (_cached) {
        console.log('[Argira] objectMap desde caché');
    ...
    console.log('[Argira] objectMap shared:', JSON.stringify(data));
...
function initCanvasTour() {
  ...
  // Panel de diagnóstico del tour — QUITAR EN PRODUCCIÓN
  let _tourDiag = document.getElementById('argira-tour-diag');
  if (!_tourDiag) {
    _tourDiag = document.createElement('pre');
    _tourDiag.id = 'argira-tour-diag';
    _tourDiag.style.cssText = [
      'background:#111', 'color:#90ee90', 'font-size:0.65rem',
      'font-family:monospace', 'padding:10px 12px', 'margin:10px 0',
      'border-radius:6px', 'border:1px solid #90ee9055',
      'white-space:pre-wrap', 'word-break:break-all',
      'max-height:320px', 'overflow-y:auto', 'line-height:1.6'
    ].join(';');
    _tourDiag.textContent = '🖼️ Tour — esperando inicio...';
    tuPanel.appendChild(_tourDiag);
  }
  window._tourDiag = _tourDiag;
  ...
  // buildTextoVoz — modo híbrido:
  //   objetoNuevo  → se incluye en la descripción (primera vez)
  //   objetoYaAnunciado → se menciona como referencia espacial, pero sin repetir el nombre completo
  function buildTextoVoz(zi, objetoNuevo, objetoRef, colorName, act) {
    const partes = [];
    partes.push(zi === 12 ? 'Centro' : ETIQUETA_ESPACIAL[zi]);
    if (colorName) partes.push(`tonos ${colorName}`);
    partes.push(actToLabel(act));
    if (objetoNuevo) {
      // Primera vez que aparece este objeto
      partes.push(objetoNuevo);
    } else if (objetoRef) {
      // Zona espacialmente relacionada con un objeto ya descrito
  ...
  async function runTour() {
    ...
    try {
      for (let idx = 0; idx < sequence.length; idx++) {
        ...
        const objetoDetectado = findObjectAtPoint(_c.nx, _c.ny, 1, 1, Array.isArray(_tuObjectMap) ? _tuObjectMap : null);
        // Modo híbrido: primera vez → anunciar; repetido → mencionar como referencia espacial
        ...
        const diagEstado = objetoDetectado
          ? (objetoNuevo ? '✨ ' + objetoDetectado : '↩ ' + objetoDetectado + ' (ref)')
          : '—';
        console.log('[ArgiraTour] STEP zi='+zi+' '+ETIQUETA_ESPACIAL[zi]+' nx='+_c.nx.toFixed(2)+' ny='+_c.ny.toFixed(2)+' -> '+diagEstado);
        // Acumular log en panel — QUITAR EN PRODUCCIÓN
        if (window._tourDiag) {
          window._tourDiag.textContent += '\n'+(idx+1)+'/'+sequence.length
            +' zi='+zi+' ['+_c.nx.toFixed(2)+','+_c.ny.toFixed(2)+'] '
            +ETIQUETA_ESPACIAL[zi]+'\n   → '+diagEstado;
          window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
        }
        const textoVoz = buildTextoVoz(zi, objetoNuevo, objetoRef, colorName, act);
        const liveRegion = document.getElementById('live-region');

        // Diagnóstico TTS — QUITAR EN PRODUCCIÓN
        if (window._tourDiag) {
          window._tourDiag.textContent += '\n   🔊 "' + textoVoz + '"';
          window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
        }
        ...
        // Enganchar callbacks ANTES de speak() para capturar START/END/ERROR
        const _diagStep = (idx + 1) + '/' + sequence.length;
        const _prevOnComplete = window.ArgiraSpeech._onComplete;
        const _origOnend = (u) => {
          if (window._tourDiag) {
            window._tourDiag.textContent += '\n   ✅ END paso ' + _diagStep;
            window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
        ...
        // Monkey-patch temporal de speak para interceptar la utterance
        const _origSpeak = window.speechSynthesis.speak.bind(window.speechSynthesis);
        let _intercepted = false;
        window.speechSynthesis.speak = function(u) {
          if (!_intercepted) {
            _intercepted = true;
            window.speechSynthesis.speak = _origSpeak; // restaurar inmediatamente
            const _prevOnend = u.onend;
            const _prevOnerror = u.onerror;
            if (window._tourDiag) {
              window._tourDiag.textContent += '\n   ▶ START paso ' + _diagStep;
              window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
            }
            u.onend = function(ev) {
              _origOnend(u);
              if (_prevOnend) _prevOnend.call(this, ev);
            };
            u.onerror = function(ev) {
              if (window._tourDiag) {
                window._tourDiag.textContent += '\n   ❌ ERROR paso ' + _diagStep + ' err=' + (ev && ev.error || '?');
                window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
              }
              if (_prevOnerror) _prevOnerror.call(this, ev);
            };
          }
          _origSpeak(u);
        };
        ...
        if (_tourSpeed === 2) {
          ...
          // Esperar señal explícita de cola terminada — no polling de .speaking
          const maxWait = _tourSpeed === 0 ? 15000 : 8000;
          await window.ArgiraSpeech.waitUntilFinished(maxWait);
  ...
  scanBtn.addEventListener('click', function() {
    ...
    window.ArgiraSpeech.stop();
    if (window._tourDiag) {
      const objList = Array.isArray(_tuObjectMap) && _tuObjectMap.length
        ? _tuObjectMap.map(o => {
            const w = ((o.x2||0)-(o.x1||0)).toFixed(2);
            const h = ((o.y2||0)-(o.y1||0)).toFixed(2);
            const area = (w*h).toFixed(2);
            return `  • ${o.label} [${(o.x1||0).toFixed(2)},${(o.y1||0).toFixed(2)}→${(o.x2||0).toFixed(2)},${(o.y2||0).toFixed(2)}] ${w}×${h} área=${area}`;
          }).join('\n')
        : '  (ninguno)';
      window._tourDiag.textContent = '🖼️ Tour iniciado\n'
        + 'Objetos API (' + (Array.isArray(_tuObjectMap) ? _tuObjectMap.length : 0) + '):\n'
        + objList + '\n\nRecorrido:\n';
    }
