...
// ── Cache global del modelo perceptual ───────────────────────────────────────
// Clave: img.src  →  Valor: buildCompatMetrics output
// Una sola ejecución del pipeline por imagen, compartida por galería, visor y
// cualquier módulo futuro. Evita O(N × pipeline) en render loops.
const ARGIRA_CARD_CACHE = new Map();

// ── Render de perfil perceptual (separado de ejecución del modelo) ───────────
// Recibe la tarjeta DOM y el objeto metrics ya calculado/cacheado.
function renderCardPerfil(card, m) {
  ...
  if (existing) existing.replaceWith(pEl);
  else {
    const rWrap = card.querySelector('.argira-roughness-widget');
    if (rWrap) rWrap.after(pEl);
  }
}
