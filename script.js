...
if (!testContainer) throw new Error('Argira: #testPairsContainer no encontrado en el DOM.');
...
function resetTest() {
  ...
  if (currentUtterance) {
    ...
    document.querySelectorAll('.speak-btn').forEach(b => {
      ...
      b.innerHTML = b._originalHTML || '🔊 Leer descripción';
    });
  }
  ...
  btn.addEventListener('click', function () {
    ...
    if (currentUtterance) {
      ...
      document.querySelectorAll('.speak-btn').forEach(b => {
        b.classList.remove('speaking');
        b.removeAttribute('aria-busy');
        b.innerHTML = b._originalHTML || originalHTML;
      });
      return;
    }
    ...
    utter.onend = () => {
      ...
      this.innerHTML = this._originalHTML || originalHTML;
    };
