  ...
  cards.forEach(card => {
    ...
    function fetchObjectMap() {
      ...
      try {
        ...
        }).then(r => r.json()).then(data => { objectMap = data; card._argiraObjectMap = data; }).catch(() => { objectMap = null; });
    ...
    scanBtn.addEventListener('click', function() {
      ...
      if (!zonesRaw) {
        ...
        const waitInterval = setInterval(() => {
          wait += 200;
          if (card.dataset.argiraClockZones || wait >= 3000) {
      ...
      const MAP_WAIT_MAX = 4000, MAP_POLL = 150;
      let mapWaited = 0;
      function startWhenReady() {
        ...
        else { mapWaited += MAP_POLL; const t = setTimeout(startWhenReady, MAP_POLL); scanTimeouts.push(t); }
      }
      startWhenReady();
    });
  });
