...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  // shrink proporcional: 8% de cada dimension de la caja, nunca destroza cajas pequeñas
  const SHRINK = 0.08;
...
function initCanvasTouch() {
  ...
  async function fetchObjectMap(imageElement) {
    try {
      ...
      console.log('[Argira] objectMap canvasTouch:', JSON.stringify(objectMap));
    } catch(e) { objectMap = null; console.warn('[Argira] canvasTouch fetch error:', e); }
  }
  ...
  window._argiraTouchInit = function(imageElement) {
    ...
    objectMap = null;
    // Esperar la señal explícita del tour en lugar de polling.
    // _argiraTourInit llama window._argiraTourMapResolve(data) cuando el fetch termina,
    // o window._argiraTourMapResolve(null) si falla.
    window._argiraTourMapPromise.then(data => {
      if (data) {
        objectMap = data;
      } else {
        // El tour falló o tardó demasiado — fetch propio de respaldo
...
function initCanvasTour() {
  ...
  let _tuObjectMap = null, _tuMapFetched = false, _tuMapSent = false;
  ...
  scanBtn.addEventListener('click', function() {
    ...
    function startWhenReady() {
      if (_aborted || _tourId !== myTourId) return;
      if (_tuMapFetched || mapWaited >= MAP_WAIT_MAX) {
        runTour();
      } else {
        if (!_vozEsperaEnviada && mapWaited >= 1500) {
          _vozEsperaEnviada = true;
  ...
  window._argiraTourInit = function(imageElement, metrics) {
    // Resetear estado
    _tuObjectMap = null; _tuMapFetched = false; _tuMapSent = false;
    _tuZones = null; _tuHues = null;
    window._argiraTuObjectMap = null;
    // Crear nueva Promise para esta imagen — el touch la espera
    window._argiraTourMapPromise = new Promise(resolve => {
      window._argiraTourMapResolve = resolve;
    });
    stopScan();
    ...
    // Fetch del mapa de objetos
    if (_tuMapSent) return;
    _tuMapSent = true;
    const tmpCanvas = document.createElement('canvas');
    ...
    fetch(PROXY_URL, {
      ...
      _tuObjectMap = data;
      _tuMapFetched = true;
      if (typeof window._argiraTourMapResolve === 'function') {
        window._argiraTourMapResolve(data);
        window._argiraTourMapResolve = null;
      }
    }).catch(() => {
      _tuMapFetched = true;
      if (typeof window._argiraTourMapResolve === 'function') {
        window._argiraTourMapResolve(null); // señal de fallo — touch hará fetch propio
        window._argiraTourMapResolve = null;
      }
    });
  };
