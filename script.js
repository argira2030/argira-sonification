...
console.log('ARGIRA SCRIPT VERSION 2026-05-31-C');
...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  // Excluir cajas de fondo (área > 50%) en ambos pasos para que océano/arena
  // nunca eclipsen objetos específicos como niña, persona, bañador.
  const SHRINK = 0.03;
  const MAX_AREA = 0.50;
  ...
  for (const obj of boxes) {
    ...
    if (w * h > MAX_AREA) continue; // saltar fondos grandes
    const sx = w * SHRINK;
    const sy = h * SHRINK;
  ...
  // --- Paso 2: fallback por proximidad (radio 15px) ---
  // Excluir cajas de fondo (área > 50% del canvas) para que océano/arena/agua
  // nunca eclipsen objetos específicos como niña, persona, bañador.
  if (!best) {
    const RADIUS_PX = 15;
    const MAX_AREA = 0.50; // cajas que cubren más del 50% son fondo
    let bestDist = RADIUS_PX;
    for (const obj of boxes) {
      const objArea = (obj.x2 - obj.x1) * (obj.y2 - obj.y1);
      if (objArea > MAX_AREA) continue; // saltar fondos grandes
    ...
    if (best) {
      console.log('[Argira] findObjectAtPoint FALLBACK borde px=%d py=%d dist=%.1f -> %s', px, py, bestDist, best.label);
    }
  }

  console.log('[Argira] findObjectAtPoint px=%d py=%d canvasW=%d canvasH=%d -> %s', px, py, canvasW, canvasH, best ? best.label : 'null');
...
function initCanvasTouch() {
  ...
  window._argiraTouchInit = function(imageElement) {
    ...
    // Panel de diagnóstico visible en móvil — borrar tras confirmar funcionamiento
    let _diagPanel = document.getElementById('argira-diag-panel');
    if (!_diagPanel) {
      _diagPanel = document.createElement('pre');
      _diagPanel.id = 'argira-diag-panel';
      _diagPanel.style.cssText = [
        'background:#111', 'color:#e8c96a', 'font-size:0.68rem',
        'font-family:monospace', 'padding:10px 12px', 'margin:10px 0',
        'border-radius:6px', 'border:1px solid #e8c96a55',
        'white-space:pre-wrap', 'word-break:break-all',
        'max-height:260px', 'overflow-y:auto', 'line-height:1.5'
      ].join(';');
      tuPanel.appendChild(_diagPanel);
    }
    _diagPanel.textContent = 'VERSION: 2026-05-31-C\nCanvas: ' +
      touchCanvas.width + '×' + touchCanvas.height +
      '\nRatio: ' + _ratio.toFixed(3) +
      '\n⏳ Esperando mapa de objetos...';
    ...
    (window._argiraTourMapPromise || Promise.resolve(null)).then(data => {
      ...
      const lines = boxes.length
        ? boxes.map((b, i) => {
            const area = ((b.x2-b.x1)*(b.y2-b.y1)).toFixed(2);
            const areaNum = parseFloat(area);
            let excluida = '';
            if (areaNum > 0.50) excluida = ' ✗fondo(área)';
            else if (areaNum > 0.30 && _BG_REGEX_DIAG.test(b.label)) excluida = ' ✗fondo(regex)';
            return `${i+1}. ${b.label}\n   área:${area}${excluida}`;
          }).join('\n')
        : '(sin objetos)';
      _diagPanel.textContent = 'VERSION: 2026-05-31-C\nCanvas: ' +
        ...
        '\nRatio: ' + _ratio.toFixed(3) +
        '\n\nObjetos recibidos:\n' + lines +
        '\n\n👆 Toca la imagen para ver selección';
    });
  };
  ...
  function handleTouch(e) {
    ...
    setTimeout(() => {
      ...
      window.ArgiraSpeech.speak(texto, { rate:0.92 });
      // Actualizar panel de diagnóstico con resultado del toque
      const _dp = document.getElementById('argira-diag-panel');
      if (_dp) {
        const normX = (px / touchCanvas.width).toFixed(2);
        const normY = (py / touchCanvas.height).toFixed(2);
        const boxes = Array.isArray(objectMap) ? objectMap : [];
        const lines = boxes.length
          ? boxes.map((b, i) => {
              const area = ((b.x2-b.x1)*(b.y2-b.y1)).toFixed(2);
              const areaNum = parseFloat(area);
              ...
              const dentro = (parseFloat(normX) >= b.x1 && parseFloat(normX) <= b.x2 && parseFloat(normY) >= b.y1 && parseFloat(normY) <= b.y2) ? ' ←dentro' : '';
              return `${i+1}. ${b.label}\n   área:${area}${excluida}${dentro}`;
            }).join('\n')
          : '(sin objetos — API pausada?)';
        _dp.textContent = 'VERSION: 2026-05-31-C' +
          '\nToque: ' + normX + ', ' + normY +
          '\nCanvas: ' + touchCanvas.width + '×' + touchCanvas.height +
          '\n\nObjetos:\n' + lines +
          '\n\nSeleccionado: ' + (objeto || 'ninguno');
      }
    }, 500);
  }
  ...
  async function runTour() {
    ...
    try {
      for (let idx = 0; idx < sequence.length; idx++) {
        ...
        const objeto = findObjectAtPoint(_c.nx, _c.ny, 1, 1, Array.isArray(_tuObjectMap) ? _tuObjectMap : null);
        const textoVoz = buildTextoVoz(zi, objeto, colorName, act);
        const liveRegion = document.getElementById('live-region');
