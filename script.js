...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    function handleGalleryTouch(e) {
      ...
      const panArrow = centroid.cx > 0.60 ? 'centro cromático-derecha' : centroid.cx > 0.53 ? 'centro cromático-centro-derecha' : centroid.cx < 0.40 ? 'centro cromático-izquierda' : centroid.cx < 0.47 ? 'centro cromático-centro-izquierda' : 'centro cromático-centro';
...
function initClockScan() {
  ...
  cards.forEach(card => {
    ...
    scanBtn.addEventListener('click', function() {
      ...
      const ZONE_CENTROIDS_XY = {
        'arriba_izquierda': [0.17, 0.17],
        'arriba_centro':    [0.50, 0.17],
        'arriba_derecha':   [0.83, 0.17],
        'centro_izquierda': [0.17, 0.50],
        'centro':           [0.50, 0.50],
        'centro_derecha':   [0.83, 0.50],
        'abajo_izquierda':  [0.17, 0.83],
        'abajo_centro':     [0.50, 0.83],
        'abajo_derecha':    [0.83, 0.83],
      };
      ...
      async function runTour() {
        // Si el mapa aún no llegó, esperar hasta 12s adicionales dentro del tour
        if (!card._argiraMapFetched) {
          const MAP_TOUR_MAX = 12000;
          let waited = 0;
          while (!card._argiraMapFetched && waited < MAP_TOUR_MAX) {
        ...
        for (let idx = 0; idx < sequence.length; idx++) {
          ...
          const objectMap = Array.isArray(card._argiraObjectMap) ? card._argiraObjectMap : null;
          const zonaKey   = ARGIRA_GRID_KEY_ZONA9[zi];
          const [cx, cy]  = ZONE_CENTROIDS_XY[zonaKey] || [0.5, 0.5];
          const half      = 0.17;
          const objeto    = objectMap
            ? (objectMap.find(o => {
                const area = (o.x2 - o.x1) * (o.y2 - o.y1);
                return area < 0.60 &&
                  o.x1 < (cx + half) && o.x2 > (cx - half) &&
                  o.y1 < (cy + half) && o.y2 > (cy - half);
              })?.label || null)
            : null;
      ...
      const MAP_WAIT_MAX = 15000, MAP_POLL = 150;
...
function initCanvasTouch() {
  ...
  function handleTouch(e) {
    ...
    const rowPos = relY < 0.33 ? 'arriba' : relY < 0.66 ? 'centro' : 'abajo';
    const posicion = (rowPos === 'centro' && colPos === 'centro') ? 'centro' : rowPos === 'centro' ? colPos : colPos === 'centro' ? rowPos : `${rowPos} ${colPos}`;
    const panArrow = relX > 0.60 ? 'centro cromático-derecha' : relX > 0.53 ? 'centro cromático-centro-derecha' : relX < 0.40 ? 'centro cromático-izquierda' : relX < 0.47 ? 'centro cromático-centro-izquierda' : 'centro cromático-centro';
    playColorTone(h,s,v);
    setTimeout(() => {
