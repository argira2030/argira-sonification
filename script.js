...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  // Cuando canvasW=1 las coordenadas son normalizadas — no aplicar mínimo absoluto
  if (!best) {
    ...
    if (best) {
      console.log('[Argira] findObjectAtPoint FALLBACK dist=%.3f -> %s', bestDist, best.label);
    }
  }
