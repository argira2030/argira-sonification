  ...
  if (liveRegion) {
    ...
    setTimeout(() => {
      liveRegion.textContent = tactileOn
        ? 'Exploración táctil activada. Toca la imagen para oír posición y color.'
        : 'Navegación accesible. El canvas táctil está desactivado.';
    }, 50);
  }

  // Recalcular la altura de cada wrap de imagen: el toggle cambia el
  // contenido visible de la tarjeta (bloque de hints, estado, etc.), lo que
  // altera wrap.offsetWidth y deja la altura fijada por syncWrapHeight()
  // obsoleta. Sin este recálculo la imagen aparece recortada/comprimida
  // hasta que el usuario redimensiona la ventana manualmente.
  if (window._argiraWrapSyncs) {
    requestAnimationFrame(() => {
      window._argiraWrapSyncs.forEach(fn => fn());
    });
  }
}
...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    // Registrar para recalcular cuando cambie el modo táctil/accesible: el
    // toggle "Exploración táctil" añade o quita contenido dentro de la tarjeta
    // y eso cambia wrap.offsetWidth, dejando la altura fijada obsoleta
    // (la imagen se ve recortada/comprimida hasta el siguiente resize manual).
    (window._argiraWrapSyncs = window._argiraWrapSyncs || []).push(syncWrapHeight);
