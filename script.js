    ...
    function handleGalleryTouch(e) {
      ...
      setTimeout(() => {
        ...
        if (firstTouch) {
          ...
          const waitTitle = new Promise(resolve => {
            const deadline = setTimeout(resolve, 5000);
            const poll = setInterval(() => {
              if (!window.ArgiraSpeech.speaking) { clearInterval(poll); clearTimeout(deadline); resolve(); }
            }, 80);
          });
          waitTitle.then(() => {
            const texto2 = objeto ? `${posicion}: ${objeto}, ${nombre}, ${freq} hercios` : `${posicion}, ${nombre}, ${freq} hercios`;
            window.ArgiraSpeech.speak(texto2, { rate: 0.92 });
          });
        } else {
          const texto = objeto ? `${posicion}: ${objeto}, ${nombre}, ${freq} hercios` : `${posicion}, ${nombre}, ${freq} hercios`;
