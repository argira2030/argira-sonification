...
function normalizeBoxes(boxes) {
  ...
  return boxes.filter(obj => {
    if (!obj || typeof obj.x1 !== 'number') return false;
    const w = obj.x2 - obj.x1;
    const h = obj.y2 - obj.y1;
    return w > 0 && h > 0 && (w * h) < 0.35; // descartar cajas absurdamente grandes
  });
}
...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  for (const obj of boxes) {
    // coordenadas normalizadas 0-1 → escalar a canvas interno
    const x1 = obj.x1 * canvasW, y1 = obj.y1 * canvasH;
    const x2 = obj.x2 * canvasW, y2 = obj.y2 * canvasH;
