...
const OBRAS = [
  ...
  { nivel: 3,  audio: 'Francisco_de_Goya,_Saturno_devorando_a_su_hijo_(1819-1823).wav',                               img: 'Francisco_de_Goya,_Saturno_devorando_a_su_hijo_(1819-1823).jpg',                               label: 'Goya · Saturno · Nivel BAJO' },
...
// Genera 3 pares aleatorios garantizando que B siempre tiene más cromatismo que A
function generarPares() {
  const shuffled = [...OBRAS].sort(() => Math.random() - 0.5);
  const pares = [];
  const usados = new Set();
  for (let i = 0; i < OBRAS.length && pares.length < 3; i++) {
    const obraA = shuffled[i];
    if (usados.has(obraA.nivel)) continue;
    // Buscar una obra B con nivel claramente mayor (diferencia mínima 3)
    const candidatasB = OBRAS.filter(o => o.nivel >= obraA.nivel + 3 && !usados.has(o.nivel));
    if (candidatasB.length === 0) continue;
    const obraB = candidatasB[Math.floor(Math.random() * candidatasB.length)];
    usados.add(obraA.nivel);
    usados.add(obraB.nivel);
    pares.push({
      pair: pares.length + 1, correct: 'B',
      audioA: obraA.audio, audioB: obraB.audio,
      imgA: obraA.img, imgB: obraB.img,
      labelA: obraA.label, labelB: obraB.label
    });
  }
  return pares;
}

const testPairs = generarPares();
