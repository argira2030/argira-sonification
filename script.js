...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  // --- Paso 1: hit test estricto con shrink del 8% ---
  // El shrink evita que un toque justo en el borde de dos cajas
  // adyacentes se asigne a la equivocada. Se elige la caja más pequeña
  // que contiene el punto (menos probable de ser fondo).
  const SHRINK = 0.08;
  ...
  // --- Paso 2: fallback por proximidad (radio 15px en espacio canvas) ---
  // Si el toque cayó fuera de todas las cajas encogidas (borde, gap entre
  // objetos, dedos grandes en móvil), buscamos la caja cuyo BORDE sea
  // más cercano al toque dentro del radio de tolerancia.
  // Distancia al borde (no al centro) es más correcta cuando dos objetos
  // están próximos: un toque cerca del borde de "niña" pero fuera de su
  // caja encogida nunca se asigna a "perro" aunque su centro esté más cerca.
  if (!best) {
    const RADIUS_PX = 15; // píxeles en el espacio del canvas interno
    let bestDist = RADIUS_PX;
    for (const obj of boxes) {
      // Coordenadas de la caja sin shrink (usamos la caja completa para el fallback)
      const bx1 = obj.x1 * canvasW, by1 = obj.y1 * canvasH;
      const bx2 = obj.x2 * canvasW, by2 = obj.y2 * canvasH;
      // Distancia euclidea al rectángulo más cercano (0 si el punto está dentro)
...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    // Prefetch silencioso cuando la card entra en pantalla
...
function initCanvasTouch() {
  ...
  // Panel de debug visible en móvil — temporal, borrar tras diagnóstico
  const _debugPanel = document.createElement('pre');
  _debugPanel.id = 'argira-debug-boxes';
  ...
  tuPanel.appendChild(_debugPanel);

  window._argiraTouchInit = function(imageElement) {
    const SIZE = 256;
    // Fix ratio — mismo cálculo que fetchSharedObjectMap
    const _ratio = imageElement.naturalWidth / imageElement.naturalHeight;
    ...
    _debugPanel.textContent = '⏳ Esperando respuesta del Worker...';
    // Consumir la Promise creada en processFile — un solo fetch para toda la página.
    (window._argiraTourMapPromise || Promise.resolve(null)).then(data => {
      objectMap = data;
      // Mostrar resultado en pantalla para diagnóstico móvil
      if (data && Array.isArray(data) && data.length) {
        const lines = data.map((b, i) =>
          `${i+1}. ${b.label}\n   x1:${b.x1.toFixed(2)} y1:${b.y1.toFixed(2)}\n   x2:${b.x2.toFixed(2)} y2:${b.y2.toFixed(2)}`
        ).join('\n\n');
        _debugPanel.textContent = `✅ ${data.length} objeto(s) detectado(s):\n\n${lines}`;
      } else if (data && data.error) {
        _debugPanel.textContent = `❌ Error Worker: ${data.error}`;
      } else {
        _debugPanel.textContent = `⚠️ Sin objetos detectados.\nRespuesta: ${JSON.stringify(data)}`;
      }
    });
  };
