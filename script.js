...
function initCanvasTouch() {
  ...
  function playColorTone(h, s, v) {
    console.log('[ArgiraAudio] playColorTone h='+h.toFixed(3)+' s='+s.toFixed(3)+' v='+v.toFixed(3));
    const ctxBefore = window.ArgiraAudio.get();
    console.log('[ArgiraAudio] estado antes de resume:', ctxBefore.state);
    window.ArgiraAudio.resume().then(function(audioCtx) {
      console.log('[ArgiraAudio] estado tras resume:', audioCtx.state);
      try {
        ...
        console.log('[ArgiraAudio] osc.start() OK freq='+freq.toFixed(1)+'Hz vol='+volume.toFixed(2));
      } catch(e) {
        console.error('[ArgiraAudio] ERROR en osc:', e.name, e.message);
      }
    }).catch(function(e) {
      console.error('[ArgiraAudio] resume() rechazado:', e);
    });
  }
  ...
  touchCanvas.addEventListener('touchstart', e => { _tuIsTouchDevice = true; _tuTouchStartY = e.touches[0].clientY; console.log('[ArgiraAudio] touchstart — pre-resumiendo'); window.ArgiraAudio.resume().then(c => console.log('[ArgiraAudio] touchstart resume OK, estado:', c.state)).catch(e => console.error('[ArgiraAudio] touchstart resume falló:', e)); }, { passive: true });
