...
function initCanvasTour() {
  ...
  function buildTextoVoz(zi, objetoNuevo, objetoRef, colorName, act, esInyectado, fondoPendiente) {
    ...
    if (objetoNuevo) {
      ...
      // Si hay fondo pendiente bajo esta referencia, fundirlo: "zona de niña, sobre arena mojada"
      if (fondoPendiente) {
        partes.push(`zona de ${objetoRef}, sobre ${fondoPendiente}`);
      } else {
    ...
    try {
      for (let idx = 0; idx < sequence.length; idx++) {
        ...
        const diagEstado = !objetoDetectado ? '—'
          ...
          : '↩ ' + objetoDetectado + ' (ref)' + (fondoPendiente ? ' + 🌅 ' + fondoPendiente : '');
