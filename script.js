...
function initCanvasTour() {
  ...
  function waitForSpeech(maxMs) {
    return new Promise(resolve => {
      const deadline = setTimeout(resolve, maxMs);
      // Esperar mínimo 400ms, luego confirmar que speaking lleva 150ms en false
      setTimeout(() => {
        let silentFor = 0;
        const poll = setInterval(() => {
          if (!window.ArgiraSpeech || !window.ArgiraSpeech.speaking) {
            ...
            silentFor = 0; // resetear si vuelve a hablar (chunk siguiente)
          }
        }, 80);
      }, 400);
    });
  ...
  async function runTour() {
    ...
    for (let idx = 0; idx < sequence.length; idx++) {
      ...
      if (_tourSpeed === 2) {
        ...
        await waitForSpeech(_tourSpeed === 0 ? 15000 : 8000);
        if (_aborted || _tourId !== myTourId) return;
