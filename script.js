...
function normalizeBoxes(boxes) {
  ...
  return boxes
    ...
    .filter(b => {
      ...
      return area < 0.45; // descarta cajas absurdamente grandes
    });
}
...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  // shrink: evita activar objetos al rozar justo el borde de la caja
  const shrink = 0.015;
  let best = null, bestArea = Infinity;
  for (const obj of boxes) {
    // coordenadas normalizadas 0-1 → escalar a canvas interno, con shrink
    const x1 = (obj.x1 + shrink) * canvasW;
    const y1 = (obj.y1 + shrink) * canvasH;
    const x2 = (obj.x2 - shrink) * canvasW;
    const y2 = (obj.y2 - shrink) * canvasH;
    if (x2 <= x1 || y2 <= y1) continue; // caja demasiado pequeña tras shrink
    if (px >= x1 && px <= x2 && py >= y1 && py <= y2) {
      // en caso de solapamiento, preferir la caja de menor área (objeto más específico)
