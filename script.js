...
// ── renderPerceptualDetails — panel <details> accesible para la Galería ──────
// Reemplaza renderCardPerfil en las tarjetas de la galería.
// Usa <details> anidados (patrón WCAG): sin JS extra, sin liveRegion duplicada.
function renderPerceptualDetails(card, m) {
  // Eliminar versión anterior si existe
  const oldDetails = card.querySelector('.argira-perceptual-details');
  ...
  // ID estable: usar el existente o generarlo a partir del índice en la galería
  let cardId = card.id;
  if (!cardId) {
    const index = Array.from(card.parentNode.children).indexOf(card);
    cardId = `artwork-${index}`;
    card.id = cardId;
  }

  // ── <details> externo: Perfil perceptual ────────────────────────────────
  const details = document.createElement('details');
  details.className = 'argira-perceptual-details';

  const summary = document.createElement('summary');
  summary.className = 'argira-perceptual-summary';
  summary.innerHTML =
    '<span><span aria-hidden="true">📊</span> Perfil perceptual de la obra</span>' +
    '<span class="argira-details-arrow" aria-hidden="true">▼</span>';
  details.appendChild(summary);

  // Contenido del panel externo
  const contentDiv = document.createElement('div');
  contentDiv.className = 'argira-perceptual-content';

  contentDiv.innerHTML =
    '<div class="argira-perfil-rows">' +
    perfil.map(p =>
      '<div class="argira-perfil-row">' +
        '<span class="argira-perfil-label"><span aria-hidden="true">' + p.icon + '</span> ' + p.label + '</span>' +
        '<span class="argira-perfil-val">' + p.val + '</span>' +
      '</div>'
    ).join('') +
    '</div>';

  // ── <details> interno: Métricas técnicas ─────────────────────────────────
  const techDetails = document.createElement('details');
  techDetails.className = 'argira-tech-details';

  const techSummary = document.createElement('summary');
  techSummary.className = 'argira-tech-summary';
  techSummary.innerHTML =
    '<span><span aria-hidden="true">📐</span> Métricas técnicas</span>' +
    '<span class="argira-details-arrow" aria-hidden="true">▼</span>';
  techDetails.appendChild(techSummary);

  const techContent = document.createElement('div');
  techContent.className = 'argira-tech-content';
  ...
  contentDiv.appendChild(techDetails);
  details.appendChild(contentDiv);

  // Insertar dentro del panel "Más formas de explorar esta obra" si existe,
  // o tras el widget de rugosidad / speak-btn como fallback
  const exploreContent = card.querySelector('.argira-explore-content');
  if (exploreContent) {
    exploreContent.appendChild(details);
  } else {
    const roughnessWidget = card.querySelector('.argira-roughness-widget');
    if (roughnessWidget) {
      roughnessWidget.insertAdjacentElement('afterend', details);
    } else {
      const speakBtn = card.querySelector('.speak-btn');
      if (speakBtn) speakBtn.insertAdjacentElement('afterend', details);
      else card.querySelector('.card-body')?.appendChild(details);
    }
  }
}

// ── renderFallbackDOM — segunda interfaz accesible para lectores de pantalla ──
// Inyecta hitos semánticos + rejilla 3×3 derivados exclusivamente de m.classified
// y spatialM.zones. No recalcula nada: consume datos ya disponibles al llamarla.
// Posición: dentro de .argira-explore-content, tras .argira-perceptual-details.
// Suspensión: CSS oculta todo el bloque en modo táctil (.argira-accessibility-mode).
function renderFallbackDOM(card, colorM, spatialM, m) {
  // Eliminar versión anterior si existe (re-render seguro)
  const old = card.querySelector('.argira-fallback-region');
  if (old) old.remove();

  const liveRegion = document.getElementById('live-region');

  // ── 1. Hitos semánticos — derivados de m (métricas clasificadas) ──────────
  // Mapeamos las 5 dimensiones del perfil perceptual a hitos artísticos.
  // El orden sigue la jerarquía: artístico primero, geométrico después.
  const T = PERCEPTUAL_THRESHOLDS;

  function hitoDesdeMetricas() {
    const hitos = [];

    // Color / saturación — hito cromático dominante
    const satNorm = Math.min(1, colorM.saturation_mean / 0.6);
    const hueDeg = colorM.hue_mean * 360;
    let colorName = 'neutro';
    if (colorM.saturation_mean >= 0.12) {
      if (hueDeg < 15 || hueDeg >= 345) colorName = 'rojo';
      else if (hueDeg < 45) colorName = 'naranja';
      else if (hueDeg < 70) colorName = 'amarillo';
      else if (hueDeg < 150) colorName = 'verde';
      else if (hueDeg < 210) colorName = 'cian';
      else if (hueDeg < 270) colorName = 'azul';
      else if (hueDeg < 330) colorName = 'violeta';
      else colorName = 'rosa';
    }
    const colorLevel = classifyPerceptual(colorM.hue_std, T.hueStd.low, T.hueStd.mid);
    hitos.push({
      icon: '🎨',
      label: `Núcleo cromático — ${colorName}`,
      level: colorLevel,
      desc: `Región de máxima identidad cromática. Tono dominante: ${colorName}. Variabilidad de color: ${colorLevel.toLowerCase()}.`,
      sonicHint: { hue: colorM.hue_mean, brightness: colorM.value_mean, complexity: colorM.hue_std }
    });

    // Luminancia / contraste — figura principal
    const lumLevel = classifyPerceptual(colorM.luminance_contrast, T.luminanceContrast.low, T.luminanceContrast.mid);
    const lumaDesc = lumLevel === 'Alto' ? 'alto contraste entre luz y sombra' : lumLevel === 'Medio' ? 'contraste moderado' : 'transiciones suaves de luz';
    hitos.push({
      icon: '☀️',
      label: `Figura principal — ${lumaDesc}`,
      level: lumLevel,
      desc: `Zona de mayor peso luminoso. ${lumaDesc.charAt(0).toUpperCase() + lumaDesc.slice(1)}.`,
      sonicHint: { hue: colorM.hue_mean, brightness: colorM.value_mean, complexity: colorM.luminance_contrast * 2 }
    });

    // Estructura / borde — tensión compositiva
    const structLevel = classifyPerceptual(colorM.edge_density, T.irregularidad.low, T.irregularidad.mid);
    const structDesc = structLevel === 'Alto' ? 'alta densidad de bordes y detalle' : structLevel === 'Medio' ? 'estructura moderada' : 'formas amplias y simples';
    hitos.push({
      icon: '🌿',
      label: `Área de tensión compositiva`,
      level: structLevel,
      desc: `Zona de mayor actividad estructural. ${structDesc.charAt(0).toUpperCase() + structDesc.slice(1)}.`,
      sonicHint: { hue: colorM.hue_mean, brightness: 0.5, complexity: colorM.edge_density }
    });

    // Complejidad fractal — fondo / textura
    const fracLevel = classifyPerceptual(m.fractalD, T.fractalD.low, T.fractalD.mid);
    const fracDesc = fracLevel === 'Alto' ? 'textura muy compleja' : fracLevel === 'Medio' ? 'textura intermedia' : 'textura lisa o uniforme';
    hitos.push({
      icon: '🔲',
      label: `Fondo dominante — ${fracDesc}`,
      level: fracLevel,
      desc: `Región de fondo. Dimensión fractal: ${fracLevel.toLowerCase()} (${m.fractalD.toFixed(3)}). ${fracDesc.charAt(0).toUpperCase() + fracDesc.slice(1)}.`,
      sonicHint: { hue: colorM.hue_mean * 0.5, brightness: 1 - colorM.value_mean, complexity: (m.fractalD - 1.5) * 2 }
    });

    return hitos;
  }

  // ── 2. Rejilla 3×3 — derivada de spatialM.zones ───────────────────────────
  const ZONE_LABELS = [
  ...
  const zones9 = spatialM.zones; // Float64Array(9), valores normalizados 0-1/9
  // Calcular brillo relativo por zona para el aria-label
  const zoneMax = Math.max(...zones9, 0.001);
  const zoneNorm = Array.from(zones9).map(v => v / zoneMax);

  // ── 3. Microsonificación regional — ~800 ms generados por AudioContext ──────
  // Se dispara en click/Enter (nunca en focus). Parámetros derivados del sonicHint.
  function microsonify(hint) {
    try {
      window.ArgiraAudio.resume().then(actx => {
        if (!actx) return;
        const dur = 0.85;
        const sr  = actx.sampleRate;

        // Frecuencia desde brillo + matiz
        const freqBase = 180 + hint.brightness * 480 + hint.hue * 220;
        // Modulación desde complejidad
        const modDepth = 30 + hint.complexity * 180;
        const modRate  = 1.5 + hint.complexity * 5;
        // Armónicos: más complejidad → más armónicos impares
        const nH = Math.floor(2 + hint.complexity * 6);
        const oddBias = Math.min(1, hint.complexity * 2);
        ...
        for (let i = 0; i < N; i++) {
          const t  = i / sr;
          const mod = modDepth * Math.sin(2 * Math.PI * modRate * t);
          let s = 0;
          for (let k = 1; k <= nH; k++) {
            let amp = 1 / Math.pow(k, 1.5);
            if (k % 2 === 0) amp *= (1 - oddBias * 0.85);
            s += amp * Math.sin(2 * Math.PI * (freqBase * k + mod) * t);
          }
          // Envolvente: ataque rápido (20ms) + fade-out suave
          const attackSamples = Math.floor(0.02 * sr);
          const env = i < attackSamples
            ? i / attackSamples
            : 1 - Math.pow((i - attackSamples) / (N - attackSamples), 1.4);
          ch[i] = s * Math.max(0, env);
        }

        // Normalizar
        let peak = 0;
        for (let i = 0; i < N; i++) if (Math.abs(ch[i]) > peak) peak = Math.abs(ch[i]);
        if (peak > 0) for (let i = 0; i < N; i++) ch[i] = ch[i] / peak * 0.72;

        const src = actx.createBufferSource();
        src.buffer = buf;
        // Pan estéreo desde matiz (izq=frío, der=cálido)
        if (actx.createStereoPanner) {
          const panner = actx.createStereoPanner();
          panner.pan.value = Math.max(-1, Math.min(1, (hint.hue - 0.5) * 1.6));
          src.connect(panner);
  ...
  region.className = 'argira-fallback-region';
  region.setAttribute('role', 'region');
  region.setAttribute('aria-label', 'Exploración accesible de la obra');
  // Modelo 2 — inicializar con aria-hidden correcto desde el primer render.
  // Evita la condición de carrera donde TalkBack detecta los botones durante
  // el intervalo entre inyección DOM y la primera llamada a applyTactileMode.
  // tabIndex se aplica a los botones internos tras construirlos (ver más abajo).
  const _tactileActive = !document.body.classList.contains('argira-accessibility-mode');
  region.setAttribute('aria-hidden', _tactileActive ? 'true' : 'false');

  const heading = document.createElement('p');
  heading.className = 'argira-fallback-heading';
  heading.textContent = 'Hitos de la obra';
  region.appendChild(heading);

  // Hitos semánticos
  const semNav = document.createElement('div');
  semNav.className = 'argira-semantic-nav';
  semNav.setAttribute('role', 'group');
  semNav.setAttribute('aria-label', 'Hitos semánticos');

  const hitos = hitoDesdeMetricas();
  hitos.forEach(h => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'argira-semantic-btn';
    btn.setAttribute('aria-label', h.desc);
    btn.setAttribute('aria-pressed', 'false');
    btn.innerHTML =
      `<span class="argira-semantic-icon" aria-hidden="true">${h.icon}</span>` +
      `<span class="argira-semantic-label">${h.label}</span>` +
      `<span class="argira-semantic-level" aria-hidden="true">${h.level}</span>`;

    btn.addEventListener('focus', () => {
      // Focus: solo describe vía live-region (sin sonido — evita batalla de voces)
      if (liveRegion) {
        liveRegion.textContent = '';
        setTimeout(() => { liveRegion.textContent = h.desc; }, 50);
      }
    });

    btn.addEventListener('click', () => {
      try { window.ArgiraAudio.resume(); } catch(e) {}
      // Quitar pressed de los demás hitos de esta tarjeta
      region.querySelectorAll('.argira-semantic-btn[aria-pressed="true"]')
            .forEach(b => { if (b !== btn) b.setAttribute('aria-pressed', 'false'); });
      region.querySelectorAll('.argira-grid-btn[aria-pressed="true"]')
            .forEach(b => b.setAttribute('aria-pressed', 'false'));
      btn.setAttribute('aria-pressed', 'true');
      // Microsonificación: ~850 ms derivados del hint de esta región
      microsonify(h.sonicHint);
    });

    semNav.appendChild(btn);
  });
  region.appendChild(semNav);

  // Rejilla 3×3
  const gridHeading = document.createElement('p');
  gridHeading.className = 'argira-spatial-heading';
  gridHeading.textContent = 'Zonas espaciales';
  region.appendChild(gridHeading);

  const gridNav = document.createElement('div');
  gridNav.className = 'argira-grid-nav';
  gridNav.setAttribute('role', 'group');
  gridNav.setAttribute('aria-label', 'Rejilla espacial 3×3');

  zoneNorm.forEach((zv, zi) => {
    const lumPct = Math.round(zv * 100);
    const lumLabel = lumPct > 70 ? 'luminosa' : lumPct > 35 ? 'media' : 'oscura';
    const zLabel = ZONE_LABELS[zi];
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'argira-grid-btn';
    btn.setAttribute('aria-label', `${zLabel} — ${lumLabel} (${lumPct}%)`);
    btn.setAttribute('aria-pressed', 'false');
    btn.innerHTML =
      `<span aria-hidden="true">${zLabel.split(' ').map(w => w.slice(0,3)).join('·')}</span><br>` +
      `<span aria-hidden="true" style="font-size:0.6rem;opacity:0.55;">${lumLabel}</span>`;

    // Calcular sonicHint para esta zona desde su luminosidad y el color global
    const zSonicHint = {
      hue: colorM.hue_mean,
      brightness: zv,               // zona más oscura → tono más grave
      complexity: colorM.edge_density * zv
    };

    btn.addEventListener('focus', () => {
      if (liveRegion) {
        liveRegion.textContent = '';
        setTimeout(() => { liveRegion.textContent = `${zLabel}: zona ${lumLabel}.`; }, 50);
      }
    });
    ...
    btn.addEventListener('click', () => {
      ...
      region.querySelectorAll('.argira-grid-btn[aria-pressed="true"]')
            .forEach(b => { if (b !== btn) b.setAttribute('aria-pressed', 'false'); });
      btn.setAttribute('aria-pressed', 'true');
      microsonify(zSonicHint);
    });

    gridNav.appendChild(btn);
  });
  region.appendChild(gridNav);

  // ── 5. tabIndex inicial — sincronizar con el estado del modo táctil ──────
  // Si el modo táctil ya estaba activo cuando se inyecta el bloque, los
  // botones nacerían con tabIndex=0 hasta que applyTactileMode los corrija.
  // Aplicarlo aquí elimina esa ventana de inconsistencia.
  if (_tactileActive) {
    const FOCUSABLES = 'button, a, input, select, textarea, [tabindex]';
    region.querySelectorAll(FOCUSABLES).forEach(btn => { btn.tabIndex = -1; });
  }

  // ── 6. Inserción en el DOM ────────────────────────────────────────────────
  // Preferencia: dentro de .argira-explore-content, tras .argira-perceptual-details.
  const exploreContent = card.querySelector('.argira-explore-content');
  if (exploreContent) {
    const perceptualDetails = exploreContent.querySelector('.argira-perceptual-details');
    if (perceptualDetails) {
      perceptualDetails.insertAdjacentElement('afterend', region);
    } else {
      exploreContent.appendChild(region);
    }
  } else {
    // Fallback: tras .argira-perceptual-details o al final del card-body
    const perceptualDetails = card.querySelector('.argira-perceptual-details');
    if (perceptualDetails) perceptualDetails.insertAdjacentElement('afterend', region);
    else card.querySelector('.card-body')?.appendChild(region);
  }
}
...
// ── Modo lector de pantalla — toggle explícito con estado persistente ─────────
// No existe API fiable para detectar TalkBack/VoiceOver desde web.
// La solución production-grade es intención del usuario, no heurística:
// el usuario activa el modo una vez, se persiste en localStorage.
// ── Estado de exploración táctil ──────────────────────────────────────────────
// Paradigma: navegación accesible es el estado BASE de Argira.
// La exploración táctil (canvas interactivo) es una función OPCIONAL que el
// usuario activa explícitamente.
//
// Clave nueva: 'argira-tactile-mode'
//   '1' = exploración táctil activa (canvas visible y con eventos)
//   '0' o null = navegación accesible (canvas oculto, gestos del lector libres)
//
// Migración: se ignora la clave antigua 'argira-reader-mode' porque su semántica
// era la inversa. Todos los usuarios arrancan en el nuevo paradigma.
(function initTactileMode() {
  const tactileOn = localStorage.getItem('argira-tactile-mode') === '1';
  // Accesibilidad activa por defecto: canvas desactivado salvo preferencia guardada
  if (!tactileOn) {
    document.body.classList.add('argira-accessibility-mode');
  }
})();

// Función global: activa o desactiva la exploración táctil.
// tactileOn = true  → canvas activo, gestos táctiles disponibles
// tactileOn = false → canvas oculto, lector de pantalla navega sin interferencias
function applyTactileMode(tactileOn) {
  // argira-accessibility-mode activo = canvas desactivado (relación inversa)
  document.body.classList.toggle('argira-accessibility-mode', !tactileOn);
  localStorage.setItem('argira-tactile-mode', tactileOn ? '1' : '0');

  // Sincronizar todos los botones de tarjeta
  document.querySelectorAll('.argira-touch-toggle').forEach(b => {
    b.setAttribute('aria-pressed', tactileOn ? 'true' : 'false');
    b.setAttribute('aria-label', tactileOn
      ? 'Exploración táctil activa. Pulsa para volver a navegación accesible.'
      : 'Activar exploración táctil: toca la imagen para oír posición y color.');
    b.querySelector('.argira-touch-toggle-label').textContent =
      tactileOn ? '👆 Exploración táctil' : '🔊 Navegación accesible';
    b.querySelector('.argira-touch-toggle-state').textContent =
      tactileOn ? 'ACTIVADA' : 'BASE';
  });

  // Modelo 2 — exclusión solo para lectores de pantalla.
  //
  // aria-hidden="true" retira el fallback DOM del árbol de accesibilidad
  // cuando el modo táctil está activo. El bloque sigue siendo visible y
  // clicable para usuarios de baja visión sin lector.
  //
  // NO se usa inert: inert + pointer-events:auto es contradictorio; los
  // navegadores pueden ignorar el override y el comportamiento futuro no
  // está garantizado.
  //
  // tabIndex: si las pruebas muestran que Tab sigue entrando en los botones
  // con aria-hidden="true", la solución es tabIndex=-1 quirúrgico (ver abajo),
  // no inert sobre el contenedor. tabIndex mantiene clic/touch y solo elimina
  // la navegación secuencial, que es exactamente lo que se necesita.
  document.querySelectorAll('.argira-fallback-region').forEach(el => {
    el.setAttribute('aria-hidden', tactileOn ? 'true' : 'false');
    // Eliminar Tab de todos los botones internos cuando el modo táctil está
    // activo. Restaurar tabIndex=0 al desactivarlo.
    // Si las pruebas confirman que aria-hidden ya bloquea Tab en los
    // navegadores objetivo, estas dos líneas pueden eliminarse.
    // Selector amplio: cubre botones actuales y cualquier elemento enfocable
    // que se añada al fallback en el futuro (enlaces, inputs, [tabindex] explícito).
    const FOCUSABLES = 'button, a, input, select, textarea, [tabindex]';
    el.querySelectorAll(FOCUSABLES).forEach(btn => {
      btn.tabIndex = tactileOn ? -1 : 0;
    });
  });
  ...
  if (liveRegion) {
    liveRegion.textContent = '';
    setTimeout(() => {
      liveRegion.textContent = tactileOn
        ? 'Exploración táctil activada. Los hitos accesibles quedan temporalmente suspendidos. Toca la imagen para oír posición y color.'
        : 'Navegación accesible restaurada. Los hitos de la obra están disponibles para explorar.';
    }, 50);
  }
}
...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    // aria-hidden: TalkBack/VoiceOver no deben entrar en el canvas táctil.
    // Los lectores de pantalla navegan por la <img> y los botones semánticos.
    // El canvas responde solo a interacción manual deliberada (Tocar/Barrido).
    tc.setAttribute('aria-hidden', 'true');
    tc.setAttribute('focusable', 'false');
    tc.tabIndex = -1;
    ...
    hintHeadphones.setAttribute('aria-hidden', 'true');
    ...
    modeBar.innerHTML = `
      <button class="argira-mode-btn argira-mode-active" data-mode="tap" aria-pressed="true" aria-label="Modo tocar: toca la imagen para oír posición y color"><span aria-hidden="true">👆</span> Tocar</button>
      <button class="argira-mode-btn" data-mode="sweep" aria-pressed="false" aria-label="Modo barrido: desliza para explorar tonos en tiempo real"><span aria-hidden="true">🖐</span> Barrido</button>
      <button class="argira-mode-btn" data-mode="braille" aria-pressed="false" aria-label="Modo braille: desliza despacio, oirás cada zona antes de avanzar"><span aria-hidden="true">🦯</span> Braille</button>`;
    ...
    const exploreDetails = document.createElement('details');
    exploreDetails.className = 'argira-explore-details';
    const exploreSummary = document.createElement('summary');
    exploreSummary.className = 'argira-explore-summary';
    exploreSummary.innerHTML =
      '<span><span aria-hidden="true">🔍</span> Más formas de explorar esta obra</span>' +
      '<span class="argira-details-arrow" aria-hidden="true">▼</span>';
    exploreDetails.appendChild(exploreSummary);
    const exploreContent = document.createElement('div');
    exploreContent.className = 'argira-explore-content';
    exploreContent.appendChild(modeBar);
    exploreContent.appendChild(modeStatus);

    // ── Botón exploración táctil ───────────────────────────────────────────
    // Estado base: navegación accesible (argira-accessibility-mode activo).
    // El botón activa/desactiva la exploración táctil opcional.
    const touchToggleBtn = document.createElement('button');
    touchToggleBtn.className = 'argira-touch-toggle';
    touchToggleBtn.type = 'button';
    const _tactileOn = !document.body.classList.contains('argira-accessibility-mode');
    touchToggleBtn.setAttribute('aria-pressed', _tactileOn ? 'true' : 'false');
    touchToggleBtn.setAttribute('aria-label', _tactileOn
      ? 'Exploración táctil activa. Pulsa para volver a navegación accesible.'
      : 'Activar exploración táctil: toca la imagen para oír posición y color.');
    // Estructura interna: etiqueta + estado — ambos actualizables desde applyTactileMode
    touchToggleBtn.innerHTML =
      '<span class="argira-touch-toggle-label">' +
        (_tactileOn ? '👆 Exploración táctil' : '🔊 Navegación accesible') +
      '</span>' +
      '<span class="argira-touch-toggle-state" aria-hidden="true">' +
        (_tactileOn ? 'ACTIVADA' : 'BASE') +
      '</span>';
    touchToggleBtn.addEventListener('click', () => {
      const nowTactile = !document.body.classList.contains('argira-accessibility-mode');
      applyTactileMode(!nowTactile);
    });
    exploreContent.appendChild(touchToggleBtn);

    exploreDetails.appendChild(exploreContent);
    wrap.after(exploreDetails);
    modeBar.querySelectorAll('.argira-mode-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        touchMode = btn.dataset.mode;
        modeBar.querySelectorAll('.argira-mode-btn').forEach(b => {
          b.classList.toggle('argira-mode-active', b === btn);
    ...
    tc.addEventListener('click', e => { if (document.body.classList.contains('argira-accessibility-mode')) return; if (!_isTouchDevice) handleGalleryTouch(e); });
    tc.addEventListener('mousedown', e => {
      if (document.body.classList.contains('argira-accessibility-mode')) return;
...
function initRoughnessWidgets() {
  ...
  function runPipelineOnCard(card, img) {
    try {
      ...
      if (ARGIRA_CACHE.has(key)) {
        ...
        renderPerceptualDetails(card, m);
        // Fallback DOM — reconstruir colorM/spatialM desde m para el cache hit
        const colorM_c  = { hue_mean: m.hueStd, saturation_mean: m.satMean, hue_std: m.hueStd, luminance_contrast: m.luminanceContrast ?? 0, edge_density: m.irregularidad, value_mean: m.valMean ?? 0.5 };
        const spatialM_c = { zones: new Float64Array(9).fill(1/9), stereo_pan: m.stereoPan ?? 0 };
        renderFallbackDOM(card, colorM_c, spatialM_c, m);
        return;
      }
      ...
      card.dataset.argiraDv     = m.fractalD.toFixed(4);
      renderPerceptualDetails(card, m);
      renderFallbackDOM(card, colorM, spatialM, m);
    } catch(e) {
  ...
  cards.forEach(card => {
    ...
    if(pct !== null) {
      ...
      rText.setAttribute('aria-hidden', 'true');
      ...
      const exploreContent = card.querySelector('.argira-explore-content');
      if (exploreContent) {
        exploreContent.insertBefore(rWrap, exploreContent.firstChild);
      } else {
...
function initCanvasTouch() {
  ...
  touchCanvas.addEventListener('click', e => { if (document.body.classList.contains('argira-accessibility-mode')) return; if (!_tuIsTouchDevice) handleTouch(e); });
  let _tuTouchStartY = 0;
  touchCanvas.addEventListener('touchstart', e => { if (document.body.classList.contains('argira-accessibility-mode')) return; _tuIsTouchDevice = true; _tuTouchStartY = e.touches[0].clientY; window.ArgiraAudio.resume(); }, { passive: true });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (stopAllBtn) {
    stopAllBtn.addEventListener('click', () => {
      ...
      stopAllBtn.setAttribute('aria-label', 'Audio detenido');
      stopAllBtn.textContent = '✓ Detenido';
      setTimeout(() => {
        stopAllBtn.innerHTML = '<span aria-hidden="true">⏹</span> Detener todo';
        stopAllBtn.setAttribute('aria-label', 'Detener todo el audio y voz');
      }, 1200);
    });
  }
});
