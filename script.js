...
window.ArgiraSpeech = {
  ...
  speak(text, opts = {}) {
    ...
    if (this.speaking && this.current && this.current.text_original === text) return Promise.resolve();
...
function processFile(file) {
  ...
  img.onload = () => {
    ...
    // Un solo fetch para ambos — Promise asignada antes de cualquier init
    window._argiraTourMapPromise = fetchSharedObjectMap(img);
    if (window._argiraTourInit) window._argiraTourInit(img, metrics);
...
function initGallerySonify() {
  // Botón "Oír la obra" y barra de ritmo eliminados.
  // Esta función ahora solo mueve audio-wrap justo después de narrBtn (Estructura espacial),
  // para que el reproductor quede en el bloque de acciones, antes del contenido descriptivo.
  const cards = document.querySelectorAll('#galeria .card');
  cards.forEach(card => {
    const narrBtn = card.querySelector('.argira-spatial-btn:not(.argira-scan-btn)');
    const audioWrap = card.querySelector('.audio-wrap');
    if (!narrBtn || !audioWrap) return;
    narrBtn.after(audioWrap);
  });
}
...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    // Decorativo: el punto de color no aporta información accesible (la voz ya anuncia posición y color).
    ...
    // role="status" + aria-live="polite": los lectores de pantalla anuncian el cambio de color
    // sin interrumpir la locución en curso (polite espera a que termine).
    // Antes colorLabel se actualizaba silenciosamente fuera del live-region global.
    colorLabel.setAttribute('role', 'status');
    colorLabel.setAttribute('aria-live', 'polite');
    colorLabel.setAttribute('aria-atomic', 'true');
    ...
    function handleGalleryTouch(e) {
      ...
      const _speakWithObject = async () => {
        const objeto = findObjectAtPoint(px, py, tc.width, tc.height,
          Array.isArray(objectMap) ? objectMap : null);
        if (firstTouch) {
          firstTouch = false;
          // await garantiza que texto2 solo arranca cuando anuncio ha terminado realmente.
          // Antes se usaba setTimeout(estimado) que no coincide con el tiempo real de sintesis,
          // provocando que speak(texto2) interrumpiera speak(anuncio) con cancel().
          await window.ArgiraSpeech.speak(anuncio, { rate: 1.0 });
          const texto2 = objeto
            ? `${posicion}, ${nombre}, ${freq} hercios, ${panArrow}, ${objeto}`
      ...
      if (Array.isArray(objectMap) || card._argiraMapFetched) {
        // clearTimeout evita que toques rapidos acumulen varios timeouts pendientes ("voces fantasma").
        clearTimeout(card._argiraSpeakTimer);
        card._argiraSpeakTimer = setTimeout(() => {
          _speakWithObject();
        ...
        const _waitMap = setInterval(() => {
          if (Array.isArray(objectMap) || card._argiraMapFetched) {
            ...
            const objeto = findObjectAtPoint(px, py, tc.width, tc.height,
              Array.isArray(objectMap) ? objectMap : null);
            // Solo hablar el objeto si no hay otra locucion en curso,
            // para evitar que _waitMap interrumpa el anuncio o el texto de posicion.
            if (objeto && !window.ArgiraSpeech.speaking) {
    ...
    // ── DIAGNÓSTICO DE EVENTOS (eliminar tras validación en dispositivo real) ──
    // Registra la secuencia completa de eventos en el canvas para analizar
    // si TalkBack/VoiceOver generan clicks sintéticos con detail===0,
    // y si pointerType/isTrusted permiten distinguirlos de clicks legítimos.
    // No modifica ningún comportamiento.
    if (window._argiraEventDiag) {
      const _diagLog = (type, e) => {
        const info = {
          type,
          detail:      e.detail,
          ...
          clientX:     Math.round(e.clientX || (e.touches && e.touches[0] ? e.touches[0].clientX : -1)),
          clientY:     Math.round(e.clientY || (e.touches && e.touches[0] ? e.touches[0].clientY : -1)),
        };
        console.log('[ArgiraEventDiag]', JSON.stringify(info));
      };
      ['click','mousedown','pointerdown','pointerup','touchstart','touchend'].forEach(t => {
        tc.addEventListener(t, e => _diagLog(t, e), { passive: true, capture: true });
      });
    }
    // ── FIN DIAGNÓSTICO ────────────────────────────────────────────────────────
...
function initRoughnessWidgets() {
  ...
  cards.forEach(card => {
    ...
    // narrBtn (Estructura espacial) se inserta después de scanBtn (Tour espacial).
    // Orden final: speak-btn → scanBtn → narrBtn → audio-wrap → contenido descriptivo.
    // scanBtn lo crea initClockScan, que corre antes que initRoughnessWidgets en DOMContentLoaded.
    // PERO initRoughnessWidgets corre ANTES que initClockScan en el nuevo orden,
    // así que aquí scanBtn aún no existe: insertamos narrBtn después de speak-btn
    // e initClockScan reordena al insertar scanBtn entre speak-btn y narrBtn.
    ...
    narrBtn.addEventListener('click', function() {
      // si ya está narrando → cancelar
      if (narrBtn.getAttribute('aria-pressed') === 'true') {
        window.ArgiraSpeech.stop();
        narrBtn.setAttribute('aria-pressed', 'false');
        return;
      }
...
function initClockScan() {
  ...
  cards.forEach(card => {
    ...
    // Orden pedido: speak-btn → scanBtn (Tour) → narrBtn (Estructura espacial)
    const _speakRef = card.querySelector('.speak-btn');
    if (_speakRef) _speakRef.after(scanBtn);
    else narrBtn.before(scanBtn);
    scanBtn.after(narrBtn);
...
window.argiraStopAll = function() {
  ...
  // reset narrBtn (Estructura espacial) si estaba activo
  document.querySelectorAll('.argira-spatial-btn:not(.argira-scan-btn)[aria-pressed="true"]').forEach(btn => { btn.setAttribute('aria-pressed', 'false'); });
...
// Orden de foco resultante por card:
// Leer descripción → Tour espacial → Estructura espacial → Escuchar sonificación
// → contenido descriptivo (artista, título, descripción, perfil sonoro, etiquetas)
// initRoughnessWidgets crea narrBtn, initClockScan reordena insertando scanBtn antes,
// initGallerySonify mueve audio-wrap después de narrBtn.
document.addEventListener('DOMContentLoaded', () => {
  initGalleryTouch();
  initGalleryAudioPan();
  initRoughnessWidgets();  // crea narrBtn (Estructura espacial)
  initClockScan();         // crea scanBtn (Tour espacial), lo inserta antes de narrBtn
  initGallerySonify();     // mueve audio-wrap después de narrBtn
  initCanvasTouch();
  initCanvasTour();
});