...
const OBRAS = [
  ...
  { nivel: 2,  audio: 'Kazimir_Malevich,_1915,_Black_Suprematic_Square,_oil_on_linen_canvas,_79.5_x_79.5_cm,_Tretyakov_Gallery,_Moscow.wav', img: 'Kazimir_Malevich,_1915,_Black_Suprematic_Square,_oil_on_linen_canvas,_79.5_x_79.5_cm,_Tretyakov_Gallery,_Moscow.jpg', label: 'Malevich · Cuadrado Negro · Nivel MUY BAJO' },
...
// 3 pares fijos con salto cromático pedagógico
const testPairs = [
  {
    pair: 1, correct: 'B',
...
// ========== Bounding boxes — utilidades globales ==========
...
// px, py: coordenadas en píxeles del toque dentro del canvas (0..canvasW, 0..canvasH)
// boxes: array normalizado de {label, x1, y1, x2, y2}
// Devuelve el label del primer objeto cuya bbox contiene el punto, o null.
function findObjectAtPoint(px, py, canvasW, canvasH, boxes) {
  if (!boxes || !boxes.length) return null;
  const nx = px / canvasW;
  const ny = py / canvasH;
  for (const b of boxes) {
    // shrink 8% de cada dimensión para evitar falsos positivos en bordes
    const shrinkX = (b.x2 - b.x1) * 0.08;
    const shrinkY = (b.y2 - b.y1) * 0.08;
    if (
      nx >= b.x1 + shrinkX && nx <= b.x2 - shrinkX &&
      ny >= b.y1 + shrinkY && ny <= b.y2 - shrinkY
    ) {
      return b.label;
    }
  }
...
// Dado un índice de zona del reloj (0-11 = horas, 12 = centro),
// devuelve el centroide normalizado {nx, ny} de esa zona sobre el canvas.
// Basado en la misma lógica de ángulo que calcCentroids().
function zonaCentroide(zi) {
  if (zi === 12) return { nx: 0.5, ny: 0.5 };
  // zi=0 → 12h (arriba), crece en sentido horario
  const angle = (zi / 12) * 2 * Math.PI - Math.PI / 2;
  const r = 0.30; // radio normalizado desde el centro
  return {
...
function initGalleryTouch() {
  ...
  cards.forEach(card => {
    ...
    const _cacheKey = 'argira_map_' + (img.src || '').split('/').pop().split('?')[0];
    function fetchObjectMap() {
      ...
      try {
        ...
        fetch(PROXY_URL, {
          ...
          try { localStorage.setItem(_cacheKey, JSON.stringify(data)); } catch(e) {}
        }).catch(() => {
          clearTimeout(_tid1);
          objectMap = null;
    ...
    function handleGalleryTouch(e) {
      ...
      setTimeout(() => {
        const _boxes = objectMap ? normalizeBoxes(objectMap) : [];
        const objeto = findObjectAtPoint(px, py, SIZE, SIZE, _boxes);
        if (firstTouch) {
          firstTouch = false;
    ...
    scanBtn.addEventListener('click', function() {
      ...
      async function runTour() {
        for (let idx = 0; idx < sequence.length; idx++) {
          ...
          const _boxes    = _rawMap ? normalizeBoxes(_rawMap) : [];
          const _c        = zonaCentroide(zi);
          const objeto    = findObjectAtPoint(_c.nx, _c.ny, 1, 1, _boxes);
...
function initCanvasTouch() {
  ...
  function handleTouch(e) {
    ...
    const rowPos = relY < 0.33 ? 'arriba' : relY < 0.66 ? 'centro' : 'abajo';
    const posicion = (rowPos === 'centro' && colPos === 'centro') ? 'centro' : rowPos === 'centro' ? colPos : colPos === 'centro' ? rowPos : `${rowPos} ${colPos}`;
    const panArrow = relX > 0.60 ? 'derecha' : relX > 0.53 ? 'centro-derecha' : relX < 0.40 ? 'izquierda' : relX < 0.47 ? 'centro-izquierda' : 'centro';
    playColorTone(h,s,v);
    setTimeout(() => {
      const _boxes = objectMap ? normalizeBoxes(objectMap) : [];
      const objeto = findObjectAtPoint(px, py, touchCanvas.width, touchCanvas.height, _boxes);
