...
function normalizeBoxes(boxes) {
  ...
  const result = boxes
    ...
    .map(b => ({
      ...b,
      x1: Math.max(0, b.x1 - 0.05),
      y1: Math.max(0, b.y1 - 0.05),
      x2: Math.min(1, b.x2 + 0.05),
      y2: Math.min(1, b.y2 + 0.05)
    }))
...
function findObjectAtPoint(px, py, canvasW, canvasH, objects) {
  ...
  if (!best) {
    const RADIUS_PX = Math.max(30, Math.round(canvasW * 0.08));
    ...
    if (best) {
      console.log('[Argira] findObjectAtPoint FALLBACK dist=%.1f -> %s', bestDist, best.label);
    }
  }
...
function initCanvasTouch() {
  ...
  window._argiraTouchInit = function(imageElement) {
    ...
    _diagPanel.textContent = 'VERSION: 2026-05-31-D\nCanvas: ' +
    ...
    (window._argiraTourMapPromise || Promise.resolve(null)).then(data => {
      ...
      const lines = boxes.length
        ? boxes.map((b, i) => {
            ...
            return `${i+1}. ${b.label}\n   [${b.x1.toFixed(2)},${b.y1.toFixed(2)} → ${b.x2.toFixed(2)},${b.y2.toFixed(2)}] área:${area}${excluida}`;
          }).join('\n')
        : '(sin objetos)';
      _diagPanel.textContent = 'VERSION: 2026-05-31-D\nCanvas: ' +
        touchCanvas.width + '×' + touchCanvas.height +
      ...
      if (_dp) {
        ...
        const lines = boxes.length
          ? boxes.map((b, i) => {
              ...
              return `${i+1}. ${b.label}\n   [${b.x1.toFixed(2)},${b.y1.toFixed(2)} → ${b.x2.toFixed(2)},${b.y2.toFixed(2)}] área:${area}${excluida}${dentro}`;
            }).join('\n')
          : '(sin objetos — API pausada?)';
        _dp.textContent = 'VERSION: 2026-05-31-D' +
          '\nToque: ' + normX + ', ' + normY +
