...
function initCanvasTour() {
  ...
  async function runTour() {
    ...
    try {
      for (let idx = 0; idx < sequence.length; idx++) {
        ...
        console.log('[ArgiraTour] STEP zi='+zi+' '+ETIQUETA_ESPACIAL[zi]+' nx='+_c.nx.toFixed(2)+' ny='+_c.ny.toFixed(2)+' -> '+(objeto||'null'));
  ...
  scanBtn.addEventListener('click', function() {
    ...
    window.ArgiraSpeech.stop();
    if (window._tourDiag) window._tourDiag.textContent = '🖼️ Tour iniciado...\n';
