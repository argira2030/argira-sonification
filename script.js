...
function initCanvasTouch() {
  ...
  window._argiraTouchInit = function(imageElement) {
    const SIZE = 256;
    // Preservar ratio — mismo cálculo que fetchSharedObjectMap
    // para que las coordenadas normalizadas de Haiku coincidan con el canvas
    const _ratio = imageElement.naturalWidth / imageElement.naturalHeight;
    ...
    (window._argiraTourMapPromise || Promise.resolve(null)).then(data => {
      objectMap = data; // null si falló — findObjectAtPoint lo maneja
    });
  };
