  ...
  cards.forEach(card => {
    ...
    scanBtn.addEventListener('click', function() {
      ...
      const ETIQUETA_ESPACIAL = [
        'Superior centro','Superior derecha','Central derecha','Central derecha',
        'Inferior derecha','Inferior centro','Inferior centro','Inferior izquierda',
        'Central izquierda','Central izquierda','Superior izquierda','Superior izquierda',
        'Centro'
      ];
      const RELOJ_A_ZONA = [
        'top-center','top-right','mid-right','mid-right','bot-right',
        'bot-center','bot-center','bot-left','mid-left','mid-left',
        'top-left','top-left','mid-center'
      ];
      ...
      async function runTour() {
        for (let idx = 0; idx < sequence.length; idx++) {
          ...
          if (_tourSpeed === 2) {
            ...
            const maxWait = _tourSpeed === 0 ? 12000 : 8000;
            await waitForSpeech(maxWait);
            if (_aborted || _tourId !== myTourId) return;
            // pausa posterior entre sectores
            const pausa = _tourSpeed === 0 ? 600 : 200;
