...
function initRoughnessWidgets() {
  ...
  cards.forEach(card => {
    ...
    // Orden de foco accesible: speak-btn -> narrBtn -> scanBtn -> sliderWrap+sonify -> contenido descriptivo.
    // narrBtn se inserta justo despues de speak-btn (primero en card-body).
    // scanBtn (initClockScan) y sliderWrap+btn (initGallerySonify) se anclan a continuacion de narrBtn.
...
// Orden importante: narrBtn y scanBtn deben existir antes de que initGallerySonify
// los busque con querySelector para anclar sliderWrap y btn (Oír la obra).
// Orden de foco resultante por card: Leer descripción → Estructura espacial →
// Tour espacial → Ritmo+Oír la obra → contenido descriptivo (artista, texto, audio).
document.addEventListener('DOMContentLoaded', () => {
  initGalleryTouch();
  initGalleryAudioPan();
  initRoughnessWidgets();  // crea narrBtn (Estructura espacial)
  initClockScan();         // crea scanBtn (Tour espacial), tras narrBtn
  initGallerySonify();     // crea sliderWrap + btn (Oír la obra), anclados a scanBtn
  initCanvasTouch();
  initCanvasTour();
});