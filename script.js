...
function initCanvasTour() {
  ...
  function buildTextoVoz(zi, objetoNuevo, objetoRef, colorName, act, esInyectado) {
  ...
  async function runTour() {
    ...
    const allLabels = Array.isArray(_tuObjectMap) ? _tuObjectMap.map(o => o.label) : [];
    ...
    try {
      for (let idx = 0; idx < sequence.length; idx++) {
        ...
        const diagEstado = !objetoDetectado ? '—'
          : objetoNuevo ? (esInyectado ? '📌 ' : '✨ ') + objetoDetectado
          : '↩ ' + objetoDetectado + ' (ref)';
        ...
        const textoVoz = buildTextoVoz(zi, objetoNuevo, objetoRef, colorName, act, esInyectado);
        const liveRegion = document.getElementById('live-region');
      ...
      if (window._tourDiag) {
        window._tourDiag.textContent += '\n\nCobertura: '
          + announced.size + '/' + allLabels.length + ' (' + cobertura + '%)';
        if (pendientesTotales.length > 0)
          window._tourDiag.textContent += '\nEpílogo: ' + pendientesTotales.join(' | ');
        window._tourDiag.scrollTop = window._tourDiag.scrollHeight;
      }
